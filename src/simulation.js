(function (root, factory) {
  ...
  function beginConflict(state, targetId, kind = 'challenge') {
    ...
    const support = state.cooperations.active.find(item => item.status === 'active' && item.expiresAt > state.clock && item.location === p.position.location && item.npcId !== targetId && state.entities[item.npcId]?.alive);
    state.combat = { kind, attacker: 'player', defender: targetId, round: 1, guard: false, startedAt: state.clock, support: support ? { cooperationId: support.id, npcId: support.npcId, used: false } : null };
    Engine.emit(state, 'combat.started', { attackerId: 'player', defenderId: targetId, kind, location: p.position.location, supportId: support?.id || null });
  ...
  function snapshot(state) {
    ...
    return {
      ...
      combat: copy(state.combat || null), rebirth: copy(state.rebirth), encounters: copy(state.encounters), cooperations: copy(state.cooperations),
