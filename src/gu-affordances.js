  ...
  function register({ engine, locations, random, clamp, remember, log, consequence, damageEntity, advance, copy }) {
    ...
    add('observeZone', {
      ...
      perform: ({ state, p }) => {
        ...
        remember(state, 'player', 'world', { kind: 'zone-observation', valence: 1, text: `你观察了${locations[p.position.location].name}的资源、危险与活动痕迹。`, facts: { observedLocation: p.position.location, danger: zone.danger, resources: copy(zone.resources) } });
        ...
        log(state, 'zone_observation', `你观察了${locations[p.position.location].name}，把这里的资源与危险写入自己的判断。`, { location: p.position.location, danger: zone.danger, resources: copy(zone.resources) });
        advance(state, 1, 'observe_zone');
    ...
    add('forage', {
      ...
      perform: ({ state, p }) => {
        ...
        if (loc === 'riverbank') {
          ...
          zone.resources.water -= amount; p.inventory.water += amount; gathered.water = amount;
        } else if (loc === 'bambooForest') {
          const petals = Math.min(2, zone.resources.moonPetal);
          if (petals < 1) throw new Error('竹林里的月兰花瓣已经被采得差不多了');
          zone.resources.moonPetal -= petals; zone.resources.food = Math.max(0, zone.resources.food - 1); p.inventory.moonPetal += petals; p.inventory.food = (p.inventory.food || 0) + 1; gathered.moonPetal = petals; gathered.food = 1;
        } else {
          const fragment = Math.min(1, zone.resources.relicFragment);
          if (fragment < 1) throw new Error('石缝里暂时没有新的遗藏碎片');
          zone.resources.relicFragment -= fragment; p.inventory.relicFragment = (p.inventory.relicFragment || 0) + fragment; state.flags.relicDiscovered = true; gathered.relicFragment = fragment;
        }
        zone.activity += 12; zone.visits = (zone.visits || 0) + 1;
        emit(state, 'world.resource_gathered', { actorId: p.id, location: loc, gathered, resources: copy(p.inventory) });
        ...
        log(state, 'environment_forage', `你在${locations[loc].name}采集了资源，区域活动和竞争痕迹同时上升。`, { location: loc, gathered });
        advance(state, 2, 'forage');
    ...
    add('searchRelic', {
      ...
      when: ({ state, p }) => {
        ...
        return tags.includes('relic') || tags.includes('inheritance') || tags.includes('hidden');
      },
      perform: ({ state, p }) => {
        const zone = zoneFor(state, p);
        const loc = p.position.location;
        ...
        if (found) {
          ...
          remember(state, 'player', 'world', { kind: 'relic-clue', valence: 3, confidence: quality, text: `你在${locations[loc].name}找到一段尚未完整的遗藏线索。`, facts: { relicClue: true, relicLocation: loc, confidence: quality } });
        } else {
          remember(state, 'player', 'world', { kind: 'failed-search', valence: -1, confidence: 0.45, text: `你在${locations[loc].name}搜索无果，但确认这里的痕迹并非自然形成。`, facts: { failedRelicSearch: true, relicLocation: loc } });
        ...
        log(state, 'relic_search', found ? `你在${locations[loc].name}发现了可以继续核验的遗藏线索。` : `你在${locations[loc].name}没有拿到遗藏，但留下了搜索痕迹。`, { location: loc, found, quality });
        advance(state, 3, 'search_relic');
    ...
    add('scoutZone', {
      ...
      perform: ({ state, p }) => {
        ...
        remember(state, 'player', 'world', { kind: 'zone-scout', valence: 2, confidence: 0.72, text: `你侦查了${locations[loc].name}，把危险、活动和可能的补给缺口记入判断。`, facts: { scoutedLocation: loc, frontId: front?.id || null, danger: zone.danger } });
        emit(state, 'zone.scouted', { actorId: p.id, location: loc, danger: zone.danger, activity: zone.activity, frontId: front?.id || null });
        log(state, 'zone_scout', `你侦查了${locations[loc].name}，获得了可以影响下一步行动的局部情报。`, { location: loc, danger: zone.danger, frontId: front?.id || null });
        advance(state, 2, 'scout_zone');
    ...
    return { definitions, available, execute };
  }
