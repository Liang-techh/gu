(function (root, factory) {
  ...
  function tick(state, { engine, day, log }) {
    ...
    const available = engine.emit(state, 'director.event_available', { eventId: candidate.id, location: player.position.location, pressure: director.pressure, source: candidate.source || candidate.id });
    candidate.provenance = available.provenance;
    log(state, 'director_event', candidate.title, { eventId: candidate.id, ruleId: selected.rule.id, score: selected.score });
    return candidate;
  }

  function resolve(state, choice, { engine, advance, onChoiceResolved }) {
    ...
    onChoiceResolved?.({ state, event, choice });
