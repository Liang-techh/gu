...
当前项目已拥有地点图、区域资源、人口、活动、危险、事件和每日推进，但 NPC 仍全部保存在同一个扁平实体表。后续应增加区域激活/挂起、区域事件队列和离线结算，把长时间世界审计从“遍历所有 NPC”推进为“活跃区域精算 + 非活跃区域摘要结算”。
...
当前项目已经有势力 influence/attitude/tension、关系网络、情报案件和市场，但市场兴趣还没有抽象成可配置的“买什么/卖什么/为什么”。这将是下一阶段把势力从数值栏升级为行为来源的重点。

## 已迁移与未迁移

已迁移：组合式实体、全局领域事件、区域资源与活动、记忆/知识、身份面具、追捕代理人、动态委托、共享市场，以及 Brain 的感知—决策—计划审计轨迹。

尚未完成：完整的组件生命周期、事件可取消/分阶段结算、区域挂起与离线结算、可配置 faction interests、真正的多层 GoalHandler 子目标执行、身体部位对能力和装备的动态约束。这些是继续完全重构时的明确工程清单。
