...
尚未完成：完整的组件生命周期、可配置 faction interests 的内容化、装备覆盖对身体部位的动态约束，以及区域实体的真正流式加载/卸载。事件阶段、可恢复的多层 GoalHandler、Body 的部位失能与蛊术约束、区域挂起与离线结算的第一版都已落地；仍需把离线摘要逐步扩展为区域级 NPC 经济、迁徙和战斗模拟，并继续补齐 Qud 式 Effect 部件。这些是继续完全重构时的明确工程清单。
