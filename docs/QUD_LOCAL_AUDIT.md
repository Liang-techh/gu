...
第一阶段已经落地：确定性世界时钟、组件式实体、事件历史式结算、NPC 日程与目标行动、记忆与关系、动态势力网络、导演事件、身体部位/伤口/冲突、自由意图解析和 JSON 存档。尚未声称完成 Qud 级别的格点物理、液体/温度模拟、完整历史生成器或完整区域生成；这些会作为后续独立系统加入，而不是假装已经实现。
