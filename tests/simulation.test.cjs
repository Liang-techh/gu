...
test('five regions use persistent war fronts with autonomous logistics and player interventions', () => {
  ...
  state.worldWar.fronts.central.pressure = 90;
  state.worldWar.fronts.central.lastActionDay = 0;
  const warNpc = S.ENTITY.createEntity('front-war-prep', { name: '战线筹备者', faction: 'centralSects', location: 'centralContinent', goals: ['prepareWar'] });
  state.entities[warNpc.id] = warNpc;
  const supplyBeforeNpc = state.worldWar.fronts.central.supply;
  assert.equal(S.ENGINE.runGoal('prepareWar', { state, npc: warNpc, faction: state.factions.centralSects }), undefined);
  assert.ok(state.worldWar.fronts.central.supply > supplyBeforeNpc);
  assert.ok(state.events.recent.some(event => event.type === 'npc.war_preparation'));
  let actions = S.ACTION_CATALOG.list(state, { locations: S.LOCATIONS });
  ...
  assert.notEqual(state.worldWar.fronts.central.supply, supplyBeforeNpc);
...
test('engine registries expose component queries, goal handlers, interactions and domain events', () => {
  ...
  assert.equal(typeof S.GU_ACTIONS.register, 'function');
  assert.equal(typeof S.GU_GOALS.register, 'function');
  ...
  assert.equal(typeof S.GU_STATE.normalize, 'function');
